...
if($fp){
	...
	while(! feof($fp)){
		...
		if(is_numeric($buffer[0])){
			...
			echo "php predict.php $time[0] $time[1] $time[2] simulate $stop_factor\n";
			shell_exec("php predict.php $time[0] $time[1] $time[2] simulate $stop_factor");
...
$outname="simulate_k_$stop_factor-$s_year$s_month$s_day-$e_year$e_month$e_day.buy";
$fp1=fopen($outname,"w");
if($fp1){
	fwrite($fp1, "{\n");
