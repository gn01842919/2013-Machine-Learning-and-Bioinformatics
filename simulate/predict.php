	...
	if($class=='2' or $class=="-2"){
		foreach($stock as $s){
			if($s->getCode()==$codeArr[$count]){//從資料編號 取得Stock 物件
				...
				*/
				global $smallestTopDV,$stop_factor;
				...
				if($DV["sign"]>0){
					if($class=="2" and $DV[2]>=$smallestTopDV){
						...
						if($stop_factor == "null"){}
						elseif($stop_factor == "random"){
							$upperbound=$s->getYesterdayClose();
							$lowerbound=$s->getYesterdayLow();
							$s->setStop(rand($upperbound, $lowerbound));
					...
					elseif($class=="-2" and $DV[-2]>=$smallestTopDV){
						...
						if($stop_factor == "null"){}
						elseif($stop_factor == "random"){
							$upperbound=$s->getYesterdayHigh();
							$lowerbound=$s->getYesterdayClose();
							$s->setStop(rand($upperbound, $lowerbound));
